...
function Btn({ ch, fn, type="def", disabled=false }){
  const styles = {
    def: { background:"rgba(255,255,255,0.1)", border:"1px solid rgba(255,255,255,0.2)", color:"#fff", borderRadius:8, padding:"7px 13px", cursor:"pointer", fontFamily:"inherit", fontSize:12 },
    pri: { background:"linear-gradient(135deg,#1ec882,#14a064)", border:"none", color:"#fff", borderRadius:8, padding:"9px 18px", cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:600, opacity:disabled?0.5:1 },
    red: { background:"rgba(220,50,50,0.2)", border:"1px solid rgba(220,80,80,0.4)", color:"#ff8080", borderRadius:8, padding:"7px 11px", cursor:"pointer", fontFamily:"inherit", fontSize:12 },
    grn: { background:"rgba(37,211,102,0.12)", border:"1px solid rgba(37,211,102,0.35)", color:"#64f096", borderRadius:8, padding:"6px 11px", cursor:"pointer", fontFamily:"inherit", fontSize:12 },
  };
...
function Emprestimos({ loans, clients, loading, reload }){
  ...
  const fbSt=(k,red)=>({background:filt===k?(red?"rgba(220,50,50,0.2)":"rgba(30,200,130,0.2)"):"transparent",border:"1px solid "+(filt===k?(red?"rgba(220,80,80,0.5)":"rgba(30,200,130,0.5)"):"rgba(255,255,255,0.15)"),borderRadius:20,color:filt===k?(red?"#ff8888":"#80ffcc"):"#888",padding:"5px 13px",cursor:"pointer",fontSize:12,fontFamily:"inherit"});
  ...
  return (
    <div>
      {alertas.length>0&&<div style={{background:"rgba(255,160,0,0.08)",border:"1px solid rgba(255,160,0,0.3)",borderRadius:12,padding:"12px 14px",marginBottom:16}}>
        <div style={{fontSize:12,fontWeight:600,color:"#ffcc50",marginBottom:8}}>🔔 Vencimentos próximos ({alertas.length})</div>
        {alertas.map(l=>{const cli=gc(l.cliente_id);const d=daysLeft(l.vencimento,l.status);return<div key={l.id} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderTop:"1px solid rgba(255,255,255,0.05)"}}>
          <span style={{fontSize:12}}>{d===0?"🔴":d<=2?"🟠":"🟡"} {cli?.nome||"—"} — {d===0?"hoje":d===1?"amanhã":d+" dias"}</span>
          <span style={{fontSize:12,fontWeight:600,color:"#ffcc50"}}>{mon(tot(l.valor,l.juros))}</span>
        </div>;})}
      </div>}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div><div style={{fontSize:18,fontWeight:700}}>Empréstimos</div><div style={{fontSize:12,color:"#666"}}>{loans.length} registro{loans.length!==1?"s":""}</div></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:16}}>
        {[{l:"Emprestado",v:loans.reduce((s,l)=>s+(parseFloat(l.valor)||0),0),c:"#fff"},{l:"A receber",v:loans.filter(l=>l.status==="pendente").reduce((s,l)=>s+tot(l.valor,l.juros),0),c:"#ffcc50"},{l:"Recebido",v:loans.filter(l=>l.status==="pago").reduce((s,l)=>s+tot(l.valor,l.juros),0),c:"#1ec882"}].map(k=>(
          <div key={k.l} style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:12,flex:1}}>
            ...
            <div style={{fontSize:13,fontWeight:700,color:k.c}}>{mon(k.v)}</div>
          </div>
        ))}
      </div>
      ...
      {filtered.map(l=>{
        ...
        return<div key={l.id} style={{...card,borderColor:vc?"rgba(220,80,80,0.4)":l.status==="pago"?"rgba(30,180,120,0.3)":"rgba(255,255,255,0.1)",background:vc?"rgba(220,50,50,0.08)":l.status==="pago"?"rgba(30,180,120,0.06)":"rgba(255,255,255,0.04)"}}>
          ...
          <div style={{display:"flex",gap:12,flexWrap:"wrap",fontSize:12,color:"#888",marginBottom:10}}>
      ...
      {show&&<Overlay onClose={()=>setShow(false)}>
        ...
        <div style={{position:"relative",marginBottom:10}}>
          ...
          {showDrop&&dropList.length>0&&<div style={{position:"absolute",width:"100%",zIndex:10,top:"100%",marginTop:4,background:"#1a1a2e",border:"1px solid rgba(255,255,255,0.15)",borderRadius:10,overflow:"hidden"}}>
            {dropList.map(c=>{const cfg=CC[c.classificacao||"neutro"];return<div key={c.id} onClick={()=>{setForm(f=>({...f,cliente_id:c.id}));setSearch(c.nome);setShowDrop(false);}} style={{padding:"10px 12px",cursor:"pointer",display:"flex",alignItems:"center",gap:8,borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
              <span>{cfg.icon}</span><div><div style={{fontSize:13}}>{c.nome}</div><div style={{fontSize:11,color:cfg.c}}>{cfg.l}</div></div>
            </div>;})}
          </div>}
        </div>
        {selCli?.classificacao==="mau"&&<div style={{background:"rgba(220,50,50,0.12)",border:"1px solid rgba(220,80,80,0.3)",borderRadius:8,padding:"8px 12px",marginBottom:10,fontSize:12,color:"#ff8888"}}>🚫 Atenção: Mau pagador!</div>}
        ...
        {form.valor&&<div style={{background:"rgba(30,200,130,0.08)",border:"1px solid rgba(30,200,130,0.2)",borderRadius:8,padding:"8px 12px",marginBottom:10,fontSize:12,color:"#aaa"}}>Total: <b style={{color:"#80ffcc"}}>{mon(tot(form.valor,form.juros))}</b></div>}
...
function Clientes({ loans, clients, loading, reload }){
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div><div style={{fontSize:18,fontWeight:700}}>Clientes</div><div style={{fontSize:12,color:"#666"}}>{clients.length} cadastrado{clients.length!==1?"s":""}</div></div>
      ...
      {filtered.map(c=>{const cfg=CC[c.classificacao||"neutro"];const ec=loans.filter(l=>l.cliente_id===c.id).length;return(
        <div key={c.id} style={{...card,display:"flex",alignItems:"center",gap:12}}>
          <div style={{width:42,height:42,borderRadius:"50%",background:cfg.bg,display:"flex",alignItems:"center",justifyContent:"center",color:cfg.c,fontWeight:700,fontSize:13,flexShrink:0}}>{initials(c.nome)}</div>
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
              <div><div style={{fontSize:14,fontWeight:700}}>{c.nome}</div><div style={{fontSize:12,color:"#666"}}>{c.telefone||"—"}{c.cpf?" · "+c.cpf:""}</div></div>
      ...
      {show&&<Overlay onClose={()=>setShow(false)}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:12}}>
          {Object.entries(CC).map(([k,v])=><button key={k} onClick={()=>setForm(f=>({...f,classificacao:k}))} style={{borderRadius:20,padding:"6px 12px",cursor:"pointer",fontSize:12,fontFamily:"inherit",background:form.classificacao===k?v.bg:"rgba(255,255,255,0.05)",border:"1px solid "+(form.classificacao===k?v.c:"rgba(255,255,255,0.1)"),color:form.classificacao===k?v.c:"#666"}}>{v.icon} {v.l}</button>)}
        </div>
...
function Relatorio({ loans, clients, loading }){
  ...
  const sh={fontSize:12,fontWeight:700,color:"#666",textTransform:"uppercase",marginBottom:10};
  ...
  return (
    <div>
      ...
      <div style={{fontSize:12,color:"#666",marginBottom:20}}>{loans.length} empréstimos · {clients.length} clientes</div>
      ...
      <div style={{marginBottom:24}}>
        ...
        {meses.map(m=>(
          <div key={m.lbl} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:10,padding:"10px 12px",marginBottom:6}}>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:6}}><b>{m.lbl}</b><span style={{color:"#555"}}>{m.cnt} empréstimo{m.cnt!==1?"s":""}</span></div>
      ...
      {VL.length>0&&<div style={{marginBottom:24}}>
        ...
        {VL.map(l=>{const c=clients.find(x=>x.id===l.cliente_id);const dias=Math.floor((new Date()-new Date(l.vencimento+"T23:59:59"))/864e5);return<div key={l.id} style={row}>
          <div><div style={{fontSize:13,fontWeight:600}}>{c?.nome||"—"}</div><div style={{fontSize:11,color:"#ff6666"}}>{dias} dia{dias!==1?"s":""} em atraso</div></div>
      ...
      {top.length>0&&<div style={{marginBottom:24}}>
        ...
        {top.map((c,i)=>{const cfg=CC[c.classificacao||"neutro"];return<div key={c.id} style={row}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <div><div style={{fontSize:13,fontWeight:600}}>{c.nome}</div><div style={{fontSize:11,color:cfg.c}}>{cfg.l}{c.indicado_por?" · via "+c.indicado_por:""}</div></div>
          </div>
...
export default function App(){
  ...
  const tSt=t=>({background:"transparent",border:"none",color:tab===t?"#fff":"#666",padding:"10px 14px",cursor:"pointer",fontSize:12,fontFamily:"inherit",fontWeight:tab===t?700:400,borderBottom:tab===t?"2px solid #1ec882":"2px solid transparent"});
